...
TreeNode* insert(TreeNode* node, int key) {
    ...
    if (balance > 1 && key < node->left->val)
        return rightRotate(node);  // Rotación simple a la derecha

    if (balance < -1 && key > node->right->val)
        return leftRotate(node);  // Rotación simple a la izquierda

    if (balance > 1 && key > node->left->val) {
        node->left = leftRotate(node->left);
        return rightRotate(node);  // Rotación doble izquierda-derecha
    }

    if (balance < -1 && key < node->right->val) {
        node->right = rightRotate(node->right);
        return leftRotate(node);  // Rotación doble derecha-izquierda
    }
